...
function plainObjectToClass(src, options) {
    ...
    const propertiesMap = schema_1.getProperties(store, { ...options, withIgnoredProps: true });
    ...
    src = alterBeforeDeserialize_1.alterBeforeDeserialize(src, store.schema, options);
    ...
    propertiesMap.forEach((propStore) => {
        ...
        if (schema_1.alterIgnore(propStore.itemSchema, options)) {
            return;
        }
        ...
        if (propStore.schema.hasGenerics) {
            ...
            if (next.type instanceof schema_1.JsonSchema) {
    ...
    return alterAfterDeserialize_1.alterAfterDeserialize(out, store.schema, options);
}
